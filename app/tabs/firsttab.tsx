...
import { spaceSchema } from "@/lib/actions";
import { z } from "zod";
...
const firsttab: React.FC<FirstTabProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Create new object URL and update state immediately
      const objectUrl = URL.createObjectURL(file);

      // Update the image URI first for immediate visual feedback
      setImageUri(objectUrl);

      // Then update the file state
    ...
    <>
      <div className="flex-1 p-8 overflow-y-auto scroll-smooth">
        ...
        <div className="space-y-7 mt-7">
          ...
          <Button onClick={handleSpaceSubmit}>Create a new space</Button>
        </div>
      </div>
    </>
  );
